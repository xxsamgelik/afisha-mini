...
- **Главная** (`/#/`) — секции по категориям из `/api/v3/pages/afisha` (как home сайта): «Рекомендуем», «Кино», «Театр»…; `typeView: horizontal` — горизонтальная лента, остальные — сетка 2 колонки; «Все» → категория; дубли между секциями убраны.
